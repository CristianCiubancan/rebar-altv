...
export function usePermissionGroup() {
    ...
    async function remove(groupName: string): Promise<boolean> {
        ...
        // Execute account and character promises first
        await Promise.all([...accountPromises, ...characterPromises]);
        
        // Then update the databases separately to avoid TypeScript issues
        // Use type assertion to fix TypeScript errors
        const pullUpdate = { $pull: { groups: groupName } } as any;
        await database.updateMany({ groups: groupName }, pullUpdate, CollectionNames.Accounts);
        await database.updateMany({ groups: groupName }, pullUpdate, CollectionNames.Characters);
        return true;
    }
